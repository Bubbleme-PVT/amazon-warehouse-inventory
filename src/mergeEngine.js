...
export async function buildMergedDashboardRows(files, settings = {}) {
  ...
  if (dashboardSheets.length && amazonSheets.length) {
    const masterRows = dashboardSheets.flatMap((sheet) => sheet.rows).map((row) => toTargetRow(row, settings.leadTimeDays || 15));
    const updatedRows = updateMasterDashboardRows(masterRows, amazonSheets, settings);
    const warnings = [
      'Master XLSX update mode used: matching rows are updated by Month + Warehouse + Product, new keys are appended, old unmatched rows are kept.'
    ];
    if (amazonSheets.some((s) => s.type === 'shipmentQueue')) warnings.push('Shipment queue CSV has warehouse totals, not product SKU rows, so open inbound is allocated across existing products for that warehouse.');
    return buildResult('Master XLSX updated from CSV files', updatedRows, loaded, warnings);
  }
...
function updateMasterDashboardRows(masterRows, sheets, settings) {
  const leadTime = settings.leadTimeDays || 15;
  const normalizedMaster = masterRows.map((row) => ({ ...row, Month: formatMonth(toMonthStart(row.Month)) }));
  const byKey = new Map(normalizedMaster.map((row) => [dashboardKey(row.Month, row.Warehouse, row.Product), { ...row }]));
  ...
  const masterMonths = unique(normalizedMaster.map((r) => r.Month)).sort();
  const latestMasterMonth = masterMonths[masterMonths.length - 1] || formatMonth(new Date());
  const masterWarehouses = unique(normalizedMaster.map((r) => r.Warehouse));
  const masterProducts = unique(normalizedMaster.map((r) => r.Product));
  const touchedSoldKeys = new Set();
  const touchedSentKeys = new Set();
  const ledgerMonths = new Set();

  for (const row of ledgerRows) {
    const monthDate = toMonthStart(getValue(row, ['Date', 'Date and Time']));
    const warehouse = cleanHeader(getValue(row, ['Fulfillment Center']));
    const product = productNameFromText(getValue(row, ['MSKU']), getValue(row, ['Title']), getValue(row, ['ASIN']));
    const qty = toNumber(getValue(row, ['Quantity']));
    const eventType = cleanHeader(getValue(row, ['Event Type'])).toLowerCase();
    if (!monthDate || !warehouse || !product) continue;
    const month = formatMonth(monthDate);
    ledgerMonths.add(month);
    const key = dashboardKey(month, warehouse, product);
    const current = byKey.get(key) || blankDashboardRow(month, warehouse, product, leadTime);
    if (qty < 0 && eventType.includes('shipment')) {
      current.Sold_Qty = toNumber(current.Sold_Qty) + Math.abs(qty);
      touchedSoldKeys.add(key);
    }
    if (qty > 0 && (eventType.includes('receipt') || eventType.includes('return') || eventType.includes('adjustment'))) {
      current.Sent_Qty = toNumber(current.Sent_Qty) + qty;
      touchedSentKeys.add(key);
    }
    byKey.set(key, current);
  }

  for (const month of ledgerMonths) {
    for (const warehouse of masterWarehouses) {
      for (const product of masterProducts) {
        const key = dashboardKey(month, warehouse, product);
        if (byKey.has(key) && !touchedSoldKeys.has(key)) byKey.get(key).Sold_Qty = 0;
        if (byKey.has(key) && !touchedSentKeys.has(key)) byKey.get(key).Sent_Qty = 0;
      }
    }
  }
  ...
  for (const row of inventoryRows) {
    ...
    const stat = inventoryByProduct.get(product) || { closing: 0, inbound: 0, snapMonth: null };
    stat.closing += toNumber(getValue(row, ['available', 'Inventory Supply at FBA']));
    stat.inbound += toNumber(getValue(row, ['inbound-quantity', 'inbound-shipped', 'inbound-receiving', 'inbound-working']));
    stat.snapMonth = toMonthStart(getValue(row, ['snapshot-date', 'Inventory age snapshot date'])) || stat.snapMonth;
    inventoryByProduct.set(product, stat);
  }

  for (const [product, stat] of inventoryByProduct) {
    const month = formatMonth(stat.snapMonth || toMonthStart(latestMasterMonth));
    distributeProductTotal(byKey, normalizedMaster, month, product, masterWarehouses, 'Closing_Stock', stat.closing, leadTime);
    if (stat.inbound > 0) distributeProductTotal(byKey, normalizedMaster, month, product, masterWarehouses, 'In_Transit', stat.inbound, leadTime);
  }

  const openShipmentByWarehouseMonth = new Map();
  for (const row of shipmentRows) {
    const warehouse = cleanHeader(getValue(row, ['Ship to']));
    const status = cleanHeader(getValue(row, ['Status'])).toLowerCase();
    const created = toMonthStart(getValue(row, ['Created', 'Last updated'])) || toMonthStart(latestMasterMonth);
    if (!warehouse || !created || !(status.includes('transit') || status.includes('receiving'))) continue;
    const openQty = Math.max(0, toNumber(getValue(row, ['Units expected'])) - toNumber(getValue(row, ['Units located'])));
    if (openQty > 0) addToMap(openShipmentByWarehouseMonth, [formatMonth(created), warehouse], openQty);
  }

  for (const [key, total] of openShipmentByWarehouseMonth.entries()) {
    const [month, warehouse] = key.split('||');
    distributeWarehouseTotal(byKey, normalizedMaster, month, warehouse, masterProducts, 'In_Transit', total, leadTime);
  }

  return [...byKey.values()].sort((a, b) => dashboardKey(a.Month, a.Warehouse, a.Product).localeCompare(dashboardKey(b.Month, b.Warehouse, b.Product)));
}

function blankDashboardRow(month, warehouse, product, leadTime) {
  return { Month: month, Warehouse: warehouse, Product: product, Sent_Qty: 0, Sold_Qty: 0, Closing_Stock: 0, In_Transit: 0, Lead_Time_Days: leadTime };
}

function dashboardKey(month, warehouse, product) {
  return [formatMonth(toMonthStart(month)), cleanHeader(warehouse), cleanHeader(product)].join('||');
}

function distributeProductTotal(byKey, masterRows, month, product, warehouses, field, total, leadTime) {
  const related = warehouses.map((warehouse) => {
    const key = dashboardKey(month, warehouse, product);
    const row = byKey.get(key) || masterRows.find((r) => dashboardKey(r.Month, r.Warehouse, r.Product) === key) || blankDashboardRow(month, warehouse, product, leadTime);
    const weight = toNumber(row[field]) || toNumber(row.Sold_Qty) || toNumber(row.Sent_Qty) || 0;
    return { warehouse, key, row, weight };
  });
  const weightTotal = related.reduce((sum, item) => sum + item.weight, 0);
  const active = weightTotal > 0 ? related.filter((item) => item.weight > 0) : related;
  active.forEach((item, index) => {
    const share = weightTotal > 0 ? item.weight / weightTotal : 1 / active.length;
    item.row[field] = index === active.length - 1
      ? Math.round(total - active.slice(0, -1).reduce((sum, prev) => sum + toNumber(prev.row[field]), 0))
      : Math.round(total * share);
    byKey.set(item.key, item.row);
  });
}

function distributeWarehouseTotal(byKey, masterRows, month, warehouse, products, field, total, leadTime) {
  const related = products.map((product) => {
    const key = dashboardKey(month, warehouse, product);
    const row = byKey.get(key) || masterRows.find((r) => dashboardKey(r.Month, r.Warehouse, r.Product) === key) || blankDashboardRow(month, warehouse, product, leadTime);
    const weight = toNumber(row[field]) || toNumber(row.Sent_Qty) || toNumber(row.Sold_Qty) || 0;
    return { product, key, row, weight };
  });
  const weightTotal = related.reduce((sum, item) => sum + item.weight, 0);
  const active = weightTotal > 0 ? related.filter((item) => item.weight > 0) : related;
  active.forEach((item, index) => {
    const share = weightTotal > 0 ? item.weight / weightTotal : 1 / active.length;
    const allocated = index === active.length - 1
      ? Math.round(total - active.slice(0, -1).reduce((sum, prev) => sum + toNumber(prev.row[field]), 0))
      : Math.round(total * share);
    item.row[field] = Math.max(toNumber(item.row[field]), allocated);
    byKey.set(item.key, item.row);
  });
}
