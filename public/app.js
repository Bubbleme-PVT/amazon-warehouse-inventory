...
async function uploadFiles(files) {
  ...
  els.dataStatus.textContent = `Uploading ${files.length} file${files.length > 1 ? 's' : ''}...`;
  ...
  state.mergedUpload = Boolean(payload.merged);
  state.sheetMap = Object.fromEntries(payload.sheets.map((sheet) => [sheet.name, sheet]));

  populateSheetOptions(payload.sheets);
  els.sheetSelect.value = payload.defaultSheetName;
  loadSheetMeta(payload.defaultSheetName);

  const totalRows = payload.sheets.find((sheet) => sheet.name === payload.defaultSheetName)?.rowCount || 0;
  renderUploadMeta({
...
async function requestBuild() {
  ...
  els.dataStatus.textContent = `${payload.meta.filteredCount} warehouse-product rows in view`;
}
...
async function exportSummary() {
  ...
  window.location.href = `/export?exportId=${encodeURIComponent(state.exportId)}`;
}
